...
namespace Game
{
	...
	void CubeTexture::Image2D(const void* data, DataType type, Format format, const Vector2u& size,
	                          InternalFormat internalFormat)
	{
		m_Owner->Bind();

		GL_CHECK(
			glTexImage2D(GetTarget(m_Face), 0, static_cast<GLint>(internalFormat), size.Width, size.Height, 0,
				static_cast<GLenum>(format), static_cast<
				GLenum>(type), data)
		);
	}

	void CubeTexture::SubImage2D(const void* data, DataType type, Format format, const Vector2i& offset,
	                             const Vector2u& size)
	{
		if (!data)
			return;

		ASSERT(offset.X + size.Width < m_Size.Width && offset.Y + size.Height < m_Size.Height,
		...
		GL_CHECK(
			glTexSubImage2D(GetTarget(m_Face), 0, offset.X, offset.Y, size.Width, size.Height, static_cast<GLenum>(
					format), static_cast<GLenum>(type), data
			)
		);
	}
