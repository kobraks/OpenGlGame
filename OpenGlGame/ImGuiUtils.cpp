...
namespace Game
{
	...
	bool ToggleButton(const std::string_view &name, bool *v)
	{
		...
		if(gg.LastActiveId == gg.CurrentWindow->GetID(id)) // && g.LastActiveIdTimer < ANIM_SPEED)
		{
		...
		if(ImGui::IsItemHovered())
			colorBG = ImGui::GetColorU32(
			                             ImLerp(ImVec4(0.78f, 0.78f, 0.78f, 1.0f), *v ? colors[ImGuiCol_ButtonActive] : ImVec4{0.78f, 0.78f, 0.78f, 1.0f}, time)
			                            );
		else
