...
namespace Game
{
	...
	void ShaderProgram::Internals::Attach(Pointer<Shader> shader)
	{
		...
		GL_LOG_INFO("Attaching {} shader {} to shader program {} (id: {})", shader->TypeToString(), shader->ID(), Name,
		            Program);
	...
	void ShaderProgram::Internals::Detach(Pointer<Shader> shader)
	{
		...
		GL_LOG_INFO("Detaching {} shader {} to shader program {} (id: {})", shader->TypeToString(), shader->ID(), Name,
		            Program);
	...
	bool ShaderProgram::Internals::IsAttached(Shader::Type type)
	{
		return std::ranges::find_if(
			Shaders, [type](const Pointer<Shader>& shader) { return type == shader->GetType(); }) != Shaders.end();
	}
	...
	void ShaderProgram::Internals::PopulateUniforms()
	{
		...
		for (uint32_t i = 0; i < count; ++i)
		{
			...
			GL_LOG_DEBUG("Uniform {}, Name: {} Size: {} Type: {}, Location: {}", i, info.Name, info.Size,
			             static_cast<uint32_t>(info.Type), info.Location);
		}
	}
	...
	{
		return *std::ranges::find_if(m_Internals->Shaders, [type](const Pointer<Shader>& shader)
		{
	...
	void ShaderProgram::BindUniformBuffer(UniformBlockIndexType index, const UniformBuffer& buffer, size_t size,
	                                      size_t offset)
	{
		if (index == INVALID_UNIFORM_BLOCK_INDEX)
			return;
	...
	{
		const auto iter = std::ranges::find_if(m_Internals->ActiveUniforms, [location](const UniformInfo& info)
		{
	...
	{
		const auto iter = std::ranges::find_if(m_Internals->ActiveUniformBlocks, [index](const UniformBlockInfo& info)
		{
