...
namespace Engine {
	Mesh::Mesh(const BufferView& vertexData, const BufferView& indexData) {
		m_IndexBuffer = IndexBuffer::Create(indexData.AsSpan<uint32_t>(), BufferStorageFlags::None);
		m_VertexBuffer = VertexBuffer::Create(vertexData.AsSpan<Vertex>(), BufferStorageFlags::None);
