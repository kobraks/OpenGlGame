...
namespace Engine {
	struct AssetMetadata {
		...
		uint64_t TimeStamp;
		std::unordered_map<std::string, std::any, TransparentStringHash> UserVariables;

		template<typename T>
		T GetOr (std::string_view index, const T& defaultValue) const {
			const auto it = UserVariables.begin();//UserVariables.find(index);
		...
		T Get(std::string_view index) const {
			const auto it = UserVariables.begin();
			if (it != UserVariables.end())
				return std::any_cast<T>(it->second);
